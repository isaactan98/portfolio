  ...
  <nav class="bg-white/30 dark:bg-zinc-900/10 fixed w-full z-[1]"
    ...
    <div class="w-full flex max-w-2xl p-2 justify-center inline-se" wrap="wrap" align="center">
      ...
      <div class="hidden sm:flex flex-row w-auto items-center grow">
        <nuxt-link v-for="n, key in nav" :key="key" class="
            p-3
            dark:text-white
            hover:underline
            underline-offset-4
            text-gray-700
          " :to="n.path" :id="n.name">{{ n.name }}</nuxt-link>
        <a target="_blank" class="
            ...
            hover:underline
            underline-offset-4
          " style="gap: 4px" href="https://github.com/isaactan98"><svg stroke="currentColor" fill="currentColor"
      ...
      <div class="flex-1" align="right">
        <ChangeThemeButton />
        <button v-if="checkauth" @click="signOut" class="rounded-lg text-sm p-2.5 shadow-lg">
          <svg class="h-5 w-5 dark:text-white" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
